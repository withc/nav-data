...
class CStartProcess(convert.feature.CWork):
    
    def __init__(self):
        convert.feature.CWork.__init__(self,  'start_pro' )
        
    def do(self):
        self.logger.info('create mid table and function') 
        self.db.run( r'.\config\mid_db.sql' )
        self.db.run( r'.\config\rdb_function.sql' )
        self.db.run( r'.\convert\%s\my.sql' % self.name )
        self._add_abbr()
    ...
    def _add_abbr(self):
        try:
            fp = open(r'.\convert\%s\abbr.txt' % self.name,'r')
        except:
            return
...
class CEndProcess(convert.feature.CWork):
    
    def __init__(self):
        convert.feature.CWork.__init__(self, 'end_pro' )
