...
class CHouseNumber(load.feature.CFeature):
    ...
    def _domake_key(self):
        sqlcmd = '''
                 ...
                 select dense_rank() over ( order by f.feat_key, f.feat_type, p.languageco, p.fullname ),
                        f.feat_key, f.feat_type, p.languageco, p.fullname, p.hno, 
                        (st_x(p.the_geom)*100000)::int, (st_y(p.the_geom)*100000)::int
                   from (
                          select hno, prov_code, amp_code, tam_code, fullname, languageco, the_geom
